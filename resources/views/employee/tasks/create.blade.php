...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('employee.tasks.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <select name="assigned_to" class="form-select @error('assigned_to') is-invalid @enderror" required>
                            ...
                            @foreach($assignableUsers as $u)
                                <option value="{{ $u->id }}" {{ (string) old('assigned_to') === (string) $u->id ? 'selected' : '' }}>{{ $u->name }}{{ $u->is_admin ? ' (Admin)' : '' }}</option>
                            @endforeach
                        </select>
